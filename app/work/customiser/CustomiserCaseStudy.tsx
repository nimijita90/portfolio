...
import { Figure, NextCase, Strip, Wip, animateKit } from "../CaseKit";
...
const features = [
  {
    ...
    body: "Clients chose a primary, secondary and tertiary colour, then opened each one’s tonal scale — every step labelled with its hex value.",
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      mm.add({ motion: "(prefers-reduced-motion: no-preference)", desktop: "(min-width: 1024px)" }, (media) => {
        ...
        gsap.timeline({ scrollTrigger: { trigger: ".cust-challenge", start: "clamp(top 86%)", once: true, invalidateOnRefresh: true } })
        ...
        gsap.utils.toArray<HTMLElement>(".cust-block").forEach((block) => {
          gsap.timeline({ scrollTrigger: { trigger: block, start: "clamp(top 84%)", once: true, invalidateOnRefresh: true } })
            .from(block.querySelector(".cust-label"), { x: -46, opacity: 0, duration: .85, ease: "power2.out" })
            .from(block.querySelector("h2"), { y: 84, opacity: 0, clipPath: "inset(0 0 80% 0)", duration: 1.3, ease: "power2.out" }, .1)
            .from(block.querySelectorAll(":scope > p:not(.cust-label), :scope > div > p:not(.cust-label), .cust-capabilities li"), { y: 30, opacity: 0, duration: .85, stagger: .05, ease: "power2.out" }, .45);
        });

        gsap.timeline({ scrollTrigger: { trigger: ".cust-process", start: "clamp(top 80%)", once: true, invalidateOnRefresh: true } })
          .from(".cust-step", { y: 60, opacity: 0, duration: 1, stagger: .14, ease: "power2.out" }, .4)
          .from(".cust-process__note", { y: 24, opacity: 0, duration: .9, ease: "power2.out" }, .9);
  ...
  return <main className="cust" id="main-content" ref={root}>
    ...
    <section className="cust-hero" aria-labelledby="cust-title">
      <div className="cust-hero__band">
        ...
        <img src={`${A}customiser-hero-banner.jpg`} width={2171} height={406} alt="Demo Casino Customiser" fetchPriority="high" />
    ...
    <section className="cust-inside cust-section" aria-labelledby="inside-title">
      <header className="cust-block"><div><p className="cust-label">Inside the Customiser</p><h2 id="inside-title">Everything a client<br />could try <em>on their own.</em></h2></div><p>Every option lived in one prototype — explored freely, without waiting on a meeting to find out what was possible.</p></header>
    ...
    <NextCase href="/work/xsite" title={<>XSITE.</>} lede="WAND’s successor, designed from the ground up for Casino, Sportsbook and every brand." banner="xsite-hero-banner.jpg" bannerWidth={2171} bannerHeight={400} />
  </main>;
}
